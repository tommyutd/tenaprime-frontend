...
if (storedLang) {
  ...
  if (
    ...
    window.userData.user.user.lang
  ) {
...
function getPageTags(path) {
  ...
  if (path === "/exercises/workout" && params.has("plan")) {
    ...
    const matches = plan.match(
      /(three-day|four-day|five-day|at-home|cardio|aerobics)/
    );
    if (matches) {
      tags.push(`${matches[1]}`);
    }
  ...
  if (path === "/exercises/strength" && params.has("group")) {
    ...
    const matches = group.match(
      /(chest|arms|shoulders|back|core|lower-body|fitness)/
    );
    if (matches) {
      tags.push(`${matches[1]}`);
    }
...
async function updatePageStrings() {
  const strings =
    window.currentLang === "en" ? window.englishStrings : window.amharicStrings;
